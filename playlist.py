...
# Create dictionary of every artists' name as the key, and a tuple of their corresponding artist_ID and top genre
artist_info = {}
...
# Set scale for SVD and load in artist IDs, playlist IDs, and the scales/weights for train/test split
# Make test size very small as Surprise SVD doesn't allow you to fit model to data, only trainset
...
# Optimal parameters after numerous grid searches
...
# One grid search example
param_grid = {'n_factors': [2, 1], 'n_epochs': [50, 60, 70], 'lr_all': [0.009, 0.01, 0.015], 'reg_all': [0.09, 0.1, 0.15]}
gs = GridSearchCV(SVD, param_grid=param_grid, cv=3, joblib_verbose=100)
gs.fit(data)
...
# Takes in artist_ID, list of playlists in that artist's top genre, and number of predictions to be outputted
# Returns list of recommended playlist IDs and their predicted ratings/estimates
...
# Gets track IDs for every song in inputted playlist
...
# Connect to Spotify API to get authorization for app
...
# Create new playlist with above name and description, and then add tracks to that playlist
if token:
...
#------------------------------------------------------------------------------------------
# Master dictionary of all artists from original data for future use:
...
#
# # Create master dictionary of artist name (lowercase for matching purposes) and the corresponding Spotify artist ID
# artist_dict = {}
